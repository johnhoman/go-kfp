...
generate: swagger
	...
	mkdir -p $(shell pwd)/api/experiment
	$(SWAGGER) generate client -f https://raw.githubusercontent.com/kubeflow/pipelines/master/backend/api/swagger/experiment.swagger.json \
		--target $(shell pwd)/api/experiment
	go mod tidy
